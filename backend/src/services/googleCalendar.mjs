...
// Safe: if not configured, just return (no crash)
export async function createCalendarEventSafe(booking) {
  ...
  if (!GOOGLE_CLIENT_EMAIL || !GOOGLE_PRIVATE_KEY || !GOOGLE_CALENDAR_ID) return
  ...
  await calendar.events.insert({
    ...
    requestBody: { summary, description, start:{ dateTime:start, timeZone: TZ||'UTC' }, end:{ dateTime:end, timeZone: TZ||'UTC' } }
  })
}
