...
import { db, addBooking, defaultSlotsFor, getBookingsForDate } from '../store.mjs'
...
router.get('/availability', (req, res) => {
  ...
  if (wd === 6) {
    ...
    for (const s of out) {
      if (addDisabled.has(s.start)) s.enabled = false
    }
  }
...
router.post('/create', (req, res) => {
  ...
  if (!full_name || !phone || !email || !vehicle) {
  ...
  if (!Array.isArray(windows) || windows.length === 0) {
    return res.status(400).json({ error: 'No windows selected' })
  }

  const rec = addBooking({
    full_name, phone, email, vehicle,
    tint_quality,
    tint_shades: Array.isArray(tint_shades) ? tint_shades : [],
    windows, date, start_time, end_time: end_time || '',
    amount_total: +amount_total || 0,
    amount_deposit: +amount_deposit || 0
  })
