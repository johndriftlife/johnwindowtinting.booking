...
export default function PaymentForm({ amount, total, bookingId, customerEmail, onSuccess }){
  ...
  useEffect(()=>{
    axios.post(`${API}/api/payments/create-payment-intent`, { amount_deposit: amount, amount_total: total, customer_email: customerEmail, metadata: { booking_id: bookingId } })
  ...
  return (
    <form onSubmit={pay}>
      <div>Total: <strong>€{(total/100).toFixed(2)}</strong> • Deposit: <strong>€{(amount/100).toFixed(2)}</strong></div>
      {clientSecret ? (<><PaymentElement /><button disabled={!stripe}>Pay deposit</button></>) : (<p>Loading payment...</p>)}
    </form>
  )
}
