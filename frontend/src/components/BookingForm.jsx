  ...
  return (
    <div style={{display:'grid', gap: 16}}>
      <div style={{textAlign:'center'}}>
        <img src={logo} alt="logo" style={{height: '16rem', width: 'auto'}} />
      ...
      <form onSubmit={submit} style={{display:'grid', gap: 12}}>
        ...
        <div style={{display:'flex', alignItems:'center', justifyContent:'space-between', padding:12, border:'1px solid #ddd', borderRadius:12}}>
          <div style={{fontSize:14}}>Total: <strong>€{(amount_total/100).toFixed(2)}</strong> • Deposit (50%): <strong>€{(amount_deposit/100).toFixed(2)}</strong></div>
          <button type="submit">Pay Deposit &amp; Book Appointment</button>
        </div>
      </form>
    </div>
  )
