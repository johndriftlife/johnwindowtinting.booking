...
export default function BookingForm({ onCreated }){
  ...
  useEffect(()=>{
    axios.get(`${API}/api/admin/shades`, { headers: { Authorization: `Bearer ${''}` }}) // requires admin token; if protected, fallback
      .then(r => { const list = r.data.filter(s=>s.quality===tint_quality && s.available===1).map(s=>s.shade); setAvailableShades(list); if(list.length) setShade(list[0]) })
  ...
  return (
    <div className="space-y-6">
      <div className="text-center space-y-3">
        <img src={logo} className="h-24 w-auto mx-auto rounded-lg" alt="logo" />
      ...
      <form className="space-y-4" onSubmit={submit}>
        ...
        <div className="flex items-center justify-between bg-neutral-900 rounded-xl p-3 border border-amber-400/20">
          <div className="text-sm text-gray-300">Total: <strong>€{(amount_total/100).toFixed(2)}</strong> • Deposit (50%): <strong>€{(amount_deposit/100).toFixed(2)}</strong></div>
