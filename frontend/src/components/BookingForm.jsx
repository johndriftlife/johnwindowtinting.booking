...
export default function BookingForm({ onCreated }) {
  ...
  const amount_total = useMemo(
    () => windows.reduce((s, w) => s + (values[w] || 0), 0),
    [windows, values]
  )
  ...
  return (
    <div className='space-y-6'>
      <div className='text-center space-y-3'>
        <img
          src={logo}
          className='h-64 w-auto mx-auto rounded-xl'
          alt='logo'
        />
      ...
      <form className='space-y-4' onSubmit={submit}>
          ...
          <div>
            ...
            <select
              ...
              onChange={e => {
                ...
                const [s, t] = val.split('-')
                const chosen = displaySlots.find(
                  x => x.start === s && x.end === t
                )
        ...
        <div className='flex items-center justify-between bg-black/60 rounded-xl p-3 border border-accent/40'>
          <div className='text-sm'>
            Total: <strong>€{(amount_total / 100).toFixed(2)}</strong> • Deposit
            (50%):{' '}
