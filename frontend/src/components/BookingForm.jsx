...
function LanguageSwitcher({lang,setLang}){ const [open,setOpen]=useState(false); const current=lang==='fr'?{code:'fr',label:'Français',flag:'🇫🇷'}:{code:'en',label:'English',flag:'🇺🇸'}; const items=[{code:'en',label:'English',flag:'🇺🇸'},{code:'fr',label:'Français',flag:'🇫🇷'}]; return (<div style={{position:'relative',display:'inline-block'}}><button type='button' onClick={()=>setOpen(o=>!o)} style={{display:'inline-flex',alignItems:'center',gap:8,background:'#111',border:'1px solid #D4AF3766',borderRadius:8,color:'#D4AF37',padding:'6px 10px'}}><span style={{fontSize:20}}>{current.flag}</span><span style={{fontSize:12}}>{current.label}</span></button>{open&&(<div style={{position:'absolute',zIndex:50,top:'110%',left:0,background:'#111',border:'1px solid #D4AF3766',borderRadius:8,overflow:'hidden'}}>{items.map(it=>(<div key={it.code} onClick={()=>{setLang(it.code);setOpen(false)}} style={{display:'flex',gap:8,alignItems:'center',padding:'8px 10px',cursor:'pointer',color:'#D4AF37'}}><span style={{fontSize:20}}>{it.flag}</span><span style={{fontSize:12}}>{it.label}</span></div>))}</div>)}</div>) }
function AdminSecretLogo({src,alt}){ const [count,setCount]=useState(0); useEffect(()=>{ if(!count) return; const t=setTimeout(()=>setCount(0),600); return ()=>clearTimeout(t) },[count]); const onClick=()=>{ const n=count+1; setCount(n); if(n>=3){ setCount(0); const key=window.prompt('Enter admin key:'); if(!key) return; if(key===import.meta.env.VITE_ADMIN_KEY){ window.location.assign('/admin') } else { alert('Invalid key') } } }; return <img src={src} alt={alt} onClick={onClick} style={{height:160,width:'auto',borderRadius:12,cursor:'pointer',display:'block',margin:'0 auto'}}/> }
...
export default function BookingForm(){
  ...
  useEffect(()=>{ axios.get(`${API}/api/public/shades`).then(res=>{ const list=(res.data&&res.data[tint_quality])? res.data[tint_quality].filter(s=>s.available).map(s=>s.shade):[]; if(list.length){ setAvailableShades(list); setTintShades(prev=>prev.filter(x=>list.includes(x))) } else { setAvailableShades(tint_quality==='carbon'?['50%','35%','20%','5%','1%']:['20%','5%']) } }).catch(()=>{ setAvailableShades(tint_quality==='carbon'?['50%','35%','20%','5%','1%']:['20%','5%']) }) },[tint_quality])
  ...
  async function submit(e){ e.preventDefault(); if(!API) return alert('Missing VITE_API_BASE'); if(!date) return alert('Choose a date'); if(!slot) return alert('Choose a time'); if(windows.length===0) return alert('Select at least one window'); if(tint_shades.length===0) return alert('Select at least one tint shade'); if(amount_deposit<=0) return alert('Deposit is zero');
    ...
    try{
      const r1=await axios.post(`${API}/api/bookings/create`,payload); const id=r1.data?.booking_id; if(!id) throw new Error('No booking_id')
      const r2=await axios.post(`${API}/api/payments/checkout`,{booking_id:id}); const url=r2.data?.url; if(!url) throw new Error('No checkout url')
      try{ window.location.assign(url); return }catch{} const a=document.createElement('a'); a.href=url; a.target='_blank'; a.rel='noopener'; document.body.appendChild(a); a.click(); a.remove()
  ...
  return (<div style={{display:'grid',gap:24}}>
    ...
    <form className='space-y-4' onSubmit={submit} style={{display:'grid',gap:16}}>
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16}}>
        ...
        <div><label style={{display:'block',marginBottom:6}}>{t('time')}</label><select value={slot?`${slot.start}-${slot.end}`:''} onChange={e=>{ const v=e.target.value; if(!v) return setSlot(null); const [s,t]=v.split('-'); const chosen=slots.find(x=>x.start===s&&x.end===t); if(!chosen?.enabled) return; setSlot({start:s,end:t}) }}><option value="">{t('select_time')}</option>{slots.slice().sort((a,b)=>a.start.localeCompare(b.start)).map((s,i)=>(<option key={i} value={`${s.start}-${s.end}`} disabled={!s.enabled}>{s.start}{s.end?` - ${s.end}`:''} {!s.enabled?`(${t('not_available')})`:''}</option>))}</select></div>
      ...
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',background:'#111',border:'1px solid #D4AF3766',borderRadius:12,padding:12}}><div style={{fontSize:14}}>{t('total')}: <strong>€{(amount_total/100).toFixed(2)}</strong> • {t('deposit')}: <strong>€{(amount_deposit/100).toFixed(2)}</strong></div><button className='btn' type='submit' disabled={submitting} style={{background:'#D4AF37',color:'#000',padding:'8px 12px',borderRadius:8,fontWeight:700}}>{submitting?'Loading payment…':t('pay_and_book')}</button></div>
    </form>
  </div>) }