...
router.post('/create', (req, res) => {
  ...
  const info = { full_name, phone, email, vehicle, tint_quality, tint_shade, windows_json: JSON.stringify(windows), date, start_time, end_time, status: 'pending_payment', payment_intent_id: null, amount_total, amount_deposit, created_at: new Date().toISOString() }
  const stmt = db.prepare("INSERT INTO bookings (full_name, phone, email, vehicle, tint_quality, tint_shade, windows_json, date, start_time, end_time, status, payment_intent_id, amount_total, amount_deposit, created_at) VALUES (@full_name, @phone, @email, @vehicle, @tint_quality, @tint_shade, @windows_json, @date, @start_time, @end_time, @status, @payment_intent_id, @amount_total, @amount_deposit, @created_at)")
